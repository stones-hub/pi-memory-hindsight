...
- Candidate project-scope authorization: a Project-scope candidate is visible and operable (custom reviewer, text list, expired view, conflict materialization, batch rejection, and direct-id approve/reject/edit-approve) only when the caller's current cwd has Project Memory enabled with an identity that exactly matches the candidate's `project_identity`; this check is enforced before any candidate claim, body edit, conflict/audit mutation, or provider I/O, including for direct-id operations naming another project's candidate. Profile-scope candidates remain visible and operable from any cwd. A matching project may retry its own previously `failed`/`reconciling` candidate. Diagnostic detail (state, project identity, and a localized explanation of `failure_code`) is shown only when the code is on a fixed allowlist of internally-constructed, provider-body-free reasons; any other code renders a generic safe message instead of the raw reason.
